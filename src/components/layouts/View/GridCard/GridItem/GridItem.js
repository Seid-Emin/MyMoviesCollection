...
const GridItem = ({
    ...
    // Link path
    let linkPath = collectionMedia ? `/collections/${status}/id=${id}` : `/${mediaType}/${filterType}/${currentPage}/id=${id}`;
                ...
                <NavLink to={linkPath} className="card-link">
                    <div className="item-image-container">
                                ...
                                <div className="card-top-rating-container">
                                    ...
                                    {collectionMedia && collectionMedia.userRating !== 'select' ?
                                        <div className="rating-star">
                                            <span
                                                className={`card-top-rating ${colorThemes.userRating[collectionMedia.userRating]}`}>{collectionMedia.userRating}</span>
                                        </div> : null}
                                </div>
                            </div> : null}
                        {/* Bottom info of card */}
                        <div className="card-bottom">
                            ...
                            {isMediaInCollection ? <span
                                className={`fl ${colorThemes.statuStyle[isMediaInCollection.watchStatus]}`}>{cardStatusConfig.title[isMediaInCollection.watchStatus]}</span> : null}
                        </div>
                        {/*  Card image */}
                        <div className="singleImg" style={{ backgroundImage: `${currentCardImage}` }}
                             onClick={() => loadSingleMedia()}></div>
                    </div>
                </NavLink>
            </div>
        </div>
